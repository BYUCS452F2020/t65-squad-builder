...
public class SquadClient extends T65Client {
    ...
    public Squad createSquad(String name) {
        URI uri = UriComponentsBuilder.fromUri(squadBuilderUrl).pathSegment("user", "{username}", "squad").build(username);
    ...
    public void deleteSquad(String squadName) {
        URI uri = UriComponentsBuilder.fromUri(squadBuilderUrl).pathSegment("user", "{username}", "squad", "{name}").build(username, squadName);
        restTemplate.delete(uri.toString());
    }

    public Squad.ShipEntry addShipToSquad(String squadName, String shipName) {
        URI uri = UriComponentsBuilder.fromUri(squadBuilderUrl).pathSegment("user", "{username}", "squad", "{name}", "ship", "{shipName}").build(username, squadName, shipName);
    ...
    public Squad.ShipEntry getShipEntry(String squadName, String shipEntryId) {
        URI uri = UriComponentsBuilder.fromUri(squadBuilderUrl).pathSegment("user", "{username}", "squad", "{name}", "ship_entry", "{shipEntryId}").build(username, squadName, shipEntryId);
        return restTemplate.postForObject(uri.toString(), null, Squad.ShipEntry.class);
    }
    ...
    public List<Squad.ShipEntry> getSquadShips(String squadName) {
        URI uri = UriComponentsBuilder.fromUri(squadBuilderUrl).pathSegment("user", "{username}", "squad", "{name}", "ships").build(username, squadName);
    ...
    public Squad.ShipEntry addUpgradeToSquadShip(String squadName, String shipEntryId, String upgradeName) {
        URI uri = UriComponentsBuilder.fromUri(squadBuilderUrl).pathSegment("user", "{username}", "squad", "{name}", "ship_entry", "{shipEntryId}", "upgrade", "{upgradeName}").build(username, squadName, shipEntryId, upgradeName);
        return restTemplate.postForObject(uri.toString(), null, Squad.ShipEntry.class);
    }
